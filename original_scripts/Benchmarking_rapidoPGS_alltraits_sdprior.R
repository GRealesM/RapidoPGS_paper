...
mbm <- microbenchmark(rapidoPGS_single(data[i],N0[i],N1[i], log.p=ifelse(i==3, TRUE,FALSE)),
	       rapidoPGS_mult(data[i], alpha.block= 1e-3, alpha.snp=0.1, sd.prior=0.2),
...
data <- data[9:10]
tests <- c("RapidoPGS_single", "RapidoPGS_multi,1e-3,0.1,customprior","RapidoPGS_multi,1e-4,0.01,customprior" )

for(i in 1:2){

mbm <- microbenchmark(rapidoPGS_single(data[i],N0,sd.prior=priors[i]),
	       rapidoPGS_mult(data[i], alpha.block= 1e-3, alpha.snp=0.1, sd.prior=priors[i]),
