...
# NOTE: BENCHMARK FOR RAPIDOPGS-MULTI AUTO PRIORS FOR QUANTITATIVE TRAITS.
...
#tests <- c("RapidoPGS_single", "RapidoPGS_multi,1e-3,0.1,autoprior","RapidoPGS_multi,1e-4,0.01,autoprior")


rapidoPGS_single <- function(data,N0,N1=NULL, sd.prior=0.2, log.p=FALSE){
ds <- fread(data)
if(is.character(ds$CHR)){
ds[,CHR:=as.numeric(CHR)]
}
full.pgs  <- computePGS(ds,N0,N1=N1, sd.prior=sd.prior, log.p=log.p)
return(full.pgs)
}
...
# for(i in 1:8){
# 
# # T1D requires log.p, and it's third element, so if i == 3 we set log.p=TRUE
# mbm <- microbenchmark(rapidoPGS_single(data[i],N0[i],N1[i], log.p=ifelse(i==3, TRUE,FALSE)),
# 	       rapidoPGS_mult(data[i], alpha.block= 1e-3, alpha.snp=0.1),
...
results <- fread("../evaluations/Results_benchmark_RapidoPGS_all_autoprior_20201115.tsv")
